...
export default function AddEditBook({ mode }) {
  ...
  useEffect(() => {
    if (mode === 'edit' && existing) {
      for (const k of ['title', 'author', 'category', 'description', 'rating']) {
        setValue(k, existing[k])
      }
    }
    ...
    <div className="max-w-2xl">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div>
          ...
          <input {...register('title')} className="w-full rounded-xl border px-3 py-2 bg-red-400" />
          {errors.title && <p className="text-red-600 text-sm">{errors.title.message}</p>}
        </div>

        <div>
          <label className="block text-sm mb-1">Author</label>
          <input {...register('author')} className="w-full rounded-xl border px-3 py-2 bg-red-400" />
          {errors.author && <p className="text-red-600 text-sm">{errors.author.message}</p>}
        </div>

        <div>
          <label className="block text-sm mb-1">Category</label>
          <input {...register('category')} list="catlist" className="w-full rounded-xl border px-3 py-2 bg-red-400" />
          <datalist id="catlist">
        ...
        <div>
          ...
          <textarea rows={5} {...register('description')} className="w-full rounded-xl border px-3 py-2 bg-red-400" />
          {errors.description && <p className="text-red-600 text-sm">{errors.description.message}</p>}
        </div>

        <div>
          <label className="block text-sm mb-1">Rating (0-5)</label>
          <input type="number" step="0.1" min="0" max="5" {...register('rating')} className="w-full rounded-xl border px-3 py-2 bg-red-400" />
          {errors.rating && <p className="text-red-600 text-sm">{errors.rating.message}</p>}
        </div>

        <div className="pt-2 flex gap-3">
          <button disabled={isSubmitting} type="submit" className="px-4 py-2 rounded-xl bg-blue-600 text-white">
            {mode === 'add' ? 'Add Book' : 'Save Changes'}
          </button>
          <button type="button" onClick={() => navigate(-1)} className="px-4 py-2 rounded-xl border">
            Cancel
          </button>
        </div>
      </form>
